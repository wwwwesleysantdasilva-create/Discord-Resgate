...
// CRIA PASTA PERSISTENTE SE NÃO EXISTIR
const dbFolder = path.join(__dirname, 'data');
if (!fs.existsSync(dbFolder)) {
    fs.mkdirSync(dbFolder, { recursive: true });
}

// CONEXÃO COM BANCO LOCAL PERSISTENTE
...
db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS products (id TEXT UNIQUE PRIMARY KEY, name TEXT, group_id TEXT)`);
...
async function enviarWebhookDiscord(mensagem) {
    ...
    try { 
        await axios.post(DISCORD_WEBHOOK_URL, { content: mensagem }, { headers: { 'Content-Type': 'application/json' } }); 
    } catch (error) { 
        console.error('Erro no webhook Discord:', error.message); 
    }
}
            ...
            if (['member', 'administrator', 'creator'].includes(newStatus) && ['left', 'kicked', 'restricted'].includes(oldStatus)) {
                db.get(`SELECT * FROM rastro_eterno WHERE telegram_id = ? ORDER BY id DESC LIMIT 1`, [telegramId], (err, registroExistente) => {
                    if (registroExistente) {
                        ...
                        db.get(`SELECT * FROM rastro_eterno WHERE (telegram_id IS NULL OR telegram_id = '') AND status_atual = 'Aguardando Entrada no Telegram' AND group_id = ? ORDER BY id DESC LIMIT 1`, [chatId], (err, ultimoGerado) => {
                            if (ultimoGerado) {
...
client.once('clientReady', async () => {
    ...
    if (TELEGRAM_TOKEN && RAILWAY_PUBLIC_DOMAIN) {
        ...
        try {
            await axios.post(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/setWebhook`, { url: webhookUrl, allowed_updates: ["chat_member", "my_chat_member"] });
            console.log(`✅ Webhook Telegram ok`);
        } catch (e) {}
    }
...
client.on('interactionCreate', async interaction => {
    if (interaction.isChatInputCommand()) {
        ...
        if (id === 'btn_add_produto') {
            ...
            db.all(`SELECT * FROM logs ORDER BY id DESC LIMIT 10`, [], (err, rows) => {
                if (!rows || !rows.length) return interaction.reply({ content: '📜 Nenhum registro.', flags: MessageFlags.Ephemeral });
                interaction.reply({ content: `📜 **Registros:**\n\n${rows.map(r => `• **[${r.timestamp}]** ${r.user}: ${r.action}`).join('\n')}`, flags: MessageFlags.Ephemeral });
            });
        }
    } 
        ...
        if (modalId === 'modal_resgate') {
            ...
            db.get(`SELECT * FROM keys WHERE key = ?`, [keyDigitada], async (err, row) => {
                ...
                db.get(`SELECT name FROM products WHERE id = ?`, [row.product], async (err, produto) => {
        ...
        if (modalId === 'modal_add_produto') {
            ...
            db.get(`SELECT * FROM products WHERE id = ?`, [prodId], (err, produto) => {
                ...
                for (let i = 0; i < qtd; i++) {
                    ...
                    db.run(`INSERT INTO keys (key, product, group_id, used, created_at) VALUES (?, ?, ?, 0, ?)`, [keyFinal, prodId, produto.group_id, dataHora]);
